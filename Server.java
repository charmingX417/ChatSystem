...
public class Server {
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("服务器"); // 创建服务器窗口
    ...
    private static void startServer() {
        new Thread(() -> {
            try (ServerSocket serverSocket = new ServerSocket(12345)) { // 创建服务器套接字
                addMessage("服务器已启动，正在监听端口...", false); // 添加启动消息
                clientSocket = serverSocket.accept(); // 接受客户端连接
                addMessage("客户端已连接！", false); // 添加连接消息

                BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream())); // 初始化输入流
                out = new PrintWriter(clientSocket.getOutputStream(), true); // 初始化输出流
                DataInputStream dataIn = new DataInputStream(clientSocket.getInputStream()); // 初始化数据输入流
                DataOutputStream dataOut = new DataOutputStream(clientSocket.getOutputStream()); // 初始化数据输出流
